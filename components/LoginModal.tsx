...
const LoginPage: React.FC<Props> = ({ isOpen, onClose, onLogin }) => {
  ...
  const fetchConfig = async (jwt: string) => {
    try {
      ...
      if (response.status === 201) {
        ...
        if (!rate.includes("%")) rate = rate + "%";
        let pitch = config.attributes.azure_pitch;
        if (pitch == undefined || pitch == "") pitch = "0";
        if (!pitch.includes("%")) pitch = pitch + "%";
        let breakms = config.attributes.breakms;
        if (breakms == undefined || breakms == "") breakms = "0";
        // 沉浸式状态
  ...
  return (
    <Modal opened={isOpen} onClose={onClose} title={t("user-login")} size="lg">
